package com.lwx.远程连接通信json_mysql_ftp_redis.redis;
 ...
 * 用图：redis的连接，增删改查，关闭
 * 可以设置key的过期时间,但是只是key,不能设置field的过期时间,因此如果只是想设置field的过期时间,只能手动删除
 */
