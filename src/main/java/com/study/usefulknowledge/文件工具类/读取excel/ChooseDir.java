        ...
        if(dir.isDirectory()) {         //文件夹部分
            ...
            if (files != null) {
                for (int i = 0; i < files.length; i++) {
                    ...
                    if (fileName.split("\\.").length == 0)fileName_extension = fileName;else {
                        fileName_extension = fileName.split("\\.")[fileName.split("\\.").length - 1];
                    }
                    if (files[i].isDirectory()) {
//                        fileNames.add(files[i].toString());
//                        getFileList(inPath,fileStyle);
//                    } else if (fileName.endsWith(fileStyle)||fileName.substring(fileName.length()-4).contains(fileStyle)) {
                    } else if (fileName.endsWith(fileStyle)||fileName_extension.contains(fileStyle)) {
                        String strFileName = files[i].getAbsolutePath();
                        fileNames.add(strFileName);
                        condition=1;
                    }else {
